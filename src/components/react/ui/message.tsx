...
export const Message: React.FC<MessageProps> = ({ variant, children }) => {
  ...
  if (variant === 'danger') {
    return (
      <div className='flex items-center  bg-red-100 border border-red-400 text-red-700 px-4 rounded relative h-full' role='alert'>
  ...
  if (variant === 'info') {
    return (
      <div className='bg-blue-50 border border-blue-300 text-blue-600 px-4 py-3 rounded relative' role='alert'>
        <span className='block sm:inline'>{children}</span>
      </div>
    );
  }
